...
acceptable_call_labels = [
    "Call",
    "Stimulus",
]  # any stimulus_trials containing call types NOT in this list are excluded (this includes unlabeled!!)
...
def call_mat_stim_trial_loader(
    ...
    stim_trials, rejected_trials, call_types = _construct_stim_trial_df(
        calls,
        file_info["wav_duration_s"],
        acceptable_call_labels,
        verbose,
    )
...
def _construct_stim_trial_df(
    calls, audio_duration, acceptable_call_labels, verbose=True
):
    """
    TODO: document
    """
...
if __name__ == "__main__":
    ...
    processed_directory = (
        "/Users/cirorandazzo/code/callback-analysis/data/processed_mats/*.mat"
    )
    acceptable_call_labels = [
        "Call",
        "Stimulus",
    ...
    for file in files:
        try:
            calls_df, stim_trials, rejected_trials, file_info, call_types = (
                call_mat_stim_trial_loader(file, verbose=False)
            )
